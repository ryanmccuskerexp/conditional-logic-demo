document.addEventListener('DOMContentLoaded', () => {
  ...
  if (toggle && builder) {
    ...
    toggle.addEventListener('change', () => {
      builder.style.display = toggle.checked ? 'block' : 'none';
      // If toggled ON and no rule blocks, add one
      const builderContainer = document.getElementById('builder-container');
      if (toggle.checked && builderContainer && builderContainer.children.length === 0) {
        addRule();
      }
    });

    // On page load, if toggle is checked and builder is empty, add a rule
    const builderContainer = document.getElementById('builder-container');
    if (toggle.checked && builderContainer && builderContainer.children.length === 0) {
      addRule();
    }
